...
项目已有可复现的三任务冻结生产版本：T1 七成员概率集成、T2/T3 SeismicXM 特征模型、统一冻结基线和历史发布证据链均已完成。第 2–6 轮候选、已有 T1 候选四口径重排以及第 7 轮基础 PhaseNet 蒸馏均未通过生产准入。正式教师缓存已覆盖 2,699 个文件，实际用时约 `466.72s`；六次固定 10 epoch LOPO 训练和六次 held-out 推理均完成。`KD-only` 的 12 单元最差/均值总分差为 `-10.4111/-2.9944`；`KD+hard` 虽有 8/12 单元上升、均值 `+6.3491`，但第 1 轮四口径全降，最差 `-6.2722`，因此两者均拒绝，生产继续冻结 `g7`。当前最高价值工作是解释 `KD+hard` 为何在 R2、08 和全部 7 个长记录上改善、却在 R1 普通短文件上回退，并在新的 AnySearch MCP + Browser MCP 研究和预注册后测试实质不同的域稳健训练机制，而不是继续调相同蒸馏超参数或做生产运维。当前服务器只承担训练、特征生成和实验计算，不作为最终部署服务器。
...
1. 汇总 `KD+hard` 在 R1 普通短文件上的回退结构，区分长度、P/S、FP/FN、信号形态和训练包组成；只做诊断，不据此事后挑超参数。
2. 使用新的 AnySearch MCP + Browser MCP 轮次检索至少 15 篇新的原始实验论文，重点研究地震相位拾取的跨域蒸馏、域平衡采样、domain generalization 和软硬标签冲突。
3. 从诊断和文献中只选择一个实质不同、可证伪的训练机制；在实现前冻结数据边界、采样规则、损失、训练预算和 12 单元准入。
4. 服务器继续只做离线实验；不部署、不修改 systemd/cron/API，不把缓存、checkpoint、预测、日志或私有审计 JSON 提交仓库。
5. T2/T3 与 gap 方向继续冻结，除非出现各自已记录的重开条件；不得与下一轮 T1 研究并行扩网格。
