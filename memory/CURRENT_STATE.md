...
1. 第 4 轮先用冻结逐文件残差和当前 scorer 做只读分组，比较 T1 多事件、超长记录、数量罚敏感性和模型分歧，选择一个有直接分数证据的问题后再开始论文/预注册；不凭兴趣扩大模型。
