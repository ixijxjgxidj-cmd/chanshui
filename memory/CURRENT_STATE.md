...
项目已有可复现的三任务冻结生产版本：T1 七成员概率集成、T2/T3 SeismicXM 特征模型、统一冻结基线和历史发布证据链均已完成。第 2–6 轮候选以及已有 T1 候选四口径重排均被否证，但第 7 轮文献与 CPU 权威基准已经把新方向收敛为同架构 PhaseNet 教师—学生蒸馏：全量教师缓存预计约 `0.0367h`、平均 float16 N/P/S 约 `62.1 MiB`、最慢单折 10 epoch 约 `0.0553h`，明确通过可行性门槛。最新本地完整回归为 `357 passed, 1 skipped`。当前最高价值工作是生成真实教师缓存并完成严格整包 LOPO 的 `KD-only`/`KD+hard` 实验，而不是继续生产运维。当前提供的服务器只承担训练、特征生成和实验计算，不作为最终部署服务器；部署、systemd、cron、服务重启和生产加固全部跳过。
