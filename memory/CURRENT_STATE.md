...
项目已经从“原型”推进到“可公开服务且有冻结证据链的三任务生产版本”：T1 七成员概率集成、T2/T3 SeismicXM 特征模型、统一冻结基线、非 root systemd 服务和公网 API 均已跑通。第 1 个“研究—实验—验证—记忆—提交”轮回已完成并部署。当前最高价值工作是完成至少两个新的严格轮回，优先解决 T3 长尾/跨包泛化和 T2 跨包校准；同时仍需补齐训练谱系、官方数量罚歧义、回滚演练和旧文档漂移。

## 已核验的生产状态

- Git 主分支：`main`。
- 当前生产提交：`53117b9d5912578c93d90e0a53774d66d0286aef`。
...
1. 启动第 2 个完整轮回：围绕 T3 长尾与跨包泛化预注册实验，重点处理 08 的 4 类 recall `20%` 以及 2/3 类弱项；开发阶段使用第 1 轮分层/嵌套验证和跨包测试，避免继续反复调 08 答案。
2. 启动第 3 个完整轮回：围绕 T2 跨包校准与分布偏移，针对 08 平均有符号误差 `+0.09978` 研究有距离/不确定性门控的残差修正；不得原样重复已失败的 quantile、Huber 和简单双模型平均。
3. 每轮继续满足 AnySearch + Playwright、至少 15 篇直接相关论文、至少 10 篇原始实验论文、预登记、统一验证、memory、GitHub 提交与推送的完整闭环。
