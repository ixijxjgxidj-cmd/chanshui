...
项目已有可复现的三任务冻结生产版本：T1 七成员概率集成、T2/T3 SeismicXM 特征模型、统一冻结基线和历史发布证据链均已完成。第 2–6 轮候选以及已有 T1 候选四口径重排均被否证，但第 7 轮文献与 CPU 权威基准已经把新方向收敛为同架构 PhaseNet 教师—学生蒸馏：全量教师缓存预计约 `0.0367h`、平均 float16 N/P/S 约 `62.1 MiB`、最慢单折 10 epoch 约 `0.0553h`，明确通过可行性门槛。正式缓存/LOPO 入口已实现并通过真实长记录烟测，最新本地完整回归为 `366 passed, 1 skipped`。当前最高价值工作是在训练服务器生成真实教师缓存并完成严格整包 LOPO 的 `KD-only`/`KD+hard` 实验，而不是继续生产运维。当前提供的服务器只承担训练、特征生成和实验计算，不作为最终部署服务器；部署、systemd、cron、服务重启和生产加固全部跳过。
...
- 当前训练服务器的职责已收窄为离线训练、特征生成和实验计算，不承担最终生产部署；后续 Git 快进仅为运行实验代码，不触发服务、systemd、cron、捕获或发布操作。
...
### 9. PhaseNet 蒸馏 CPU 可行性已通过，正式 LOPO 尚未运行

提交 `a846c26118195f8c63cb8d26f0cfe5ebac3bd7bc` 的权威 CPU 基准完整盘点三套包 2,699 个 T1 文件、232,206.05 秒波形和 7 个长文件；代表 annotation 确认 N/P/S 三通道、50 Hz 概率网格和正确长记录长度。全量冻结教师平均概率缓存预计 `132.1285s = 0.036702h`，float16 体积 `65,137,386` 字节。

同架构 `PhaseNet(diting)` 的 CPU forward/backward/optimizer step 平均 `0.008471s`，loss `0.302985→0.216054`，峰值 RSS `702,156,800` 字节。按 60.02 秒窗、30 秒 stride、batch 2、固定 10 epochs，最慢 LOPO 折预计 `0.055274h`。权威 JSON SHA-256 为 `7a6a46473d05d5f78fcd2440db8c1643f46142ba839c25782ceaece22b6e43be`，权限 `0600`；它只证明计算可行，没有生成正式缓存、学生权重或分数。

下一实验固定比较 `KD-only` 与 `0.7 KD + 0.3 hard`，严格整包留出，不用 held-out early stopping，不扫温度/alpha/epoch/成员权重。三折 OOP 预测必须运行完整生产后处理，并在三包 × 四数量罚 12 单元全部不下降且至少一项严格提升时才录取。完整预注册见 `memory/papers/round-07-t1-teacher-student-distillation.md` 与 `memory/experiments/011-t1-cpu-distillation-feasibility.md`。
...
1. 实现并测试教师平均概率缓存入口；缓存只保留 float16 N/P/S、文件身份、时间轴和必要元数据，不保存七成员曲线，不写比赛数据机器绝对路径。
2. 实现固定三折 LOPO 训练入口，训练 `KD-only` 与 `KD+hard`；学生、batch、epoch、优化器、学习率、stride、hard-label sigma 和随机种子在运行前冻结。
3. 在训练服务器生成真实缓存并完成六个训练运行；服务器不做最终部署，不修改或重启任何服务。
4. 每折训练完成后才运行 held-out 推理；拼接三包 OOP 预测，复用生产完整后处理与评分器报告 12 单元、逐文件回归、FP/FN、P/S 时差分和长文件。
5. 若任一候选满足 12 单元全部不下降且至少一项严格提升，再进入独立复核；若两者均失败，记录负结果，不围绕同一历史包继续调温度、alpha、epoch、成员权重或阈值。
6. T2/T3 与 gap 方向继续冻结，除非出现各自已记录的重开条件；它们不与本轮蒸馏并行扩网格。
