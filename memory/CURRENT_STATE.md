...
项目已经从“原型”推进到“可公开服务且有冻结证据链的三任务生产版本”：T1 七成员概率集成、T2/T3 SeismicXM 特征模型、统一冻结基线、非 root systemd 服务和公网 API 均已跑通。第 1 轮短窗摄入修复已完成并部署；第 2 轮 T3 长尾轻量头因跨包从 `187/189` 崩至 `121/189` 被拒绝；第 3 轮 T2 source-only OOF residual 校准虽在两个源包内部均上涨，但跨包 MAE 分别恶化 `0.00049/0.00889`，也被严格拒绝。第 4 轮已完成 16 篇长记录事件关联/联合检测论文核验与实验预注册，目标是在现有 20 秒去重后只删除完整低可信 P/S 假事件；阈值评分尚未运行，生产与服务器保持不变。三个完整闭环已经达到最低轮次数，但停止条件尚未满足。
...
本轮已同时使用 AnySearch 与 Playwright 核验 16 篇新的原始实验研究。成熟 PhaseLink/GaMMA/PyOcto/GENIE 等方法依赖多台站、位置和速度模型，当前单站输入不能直接使用；EQTransformer/CRED/PhaseNet+ 等支持“完整事件证据优于独立相位峰值”。已冻结实验：只处理 `>300s`，固定生产 20 秒去重、FIFO `0.2–60s` P→S 配对、几何联合分数和 tau `{0.35,0.40,0.45}`；R2/08 各自选型、双向跨包、较低 tau 作为唯一共同候选，且要求四口径、逐文件 FN/时差分和短文件逐位一致性全部通过。论文与预注册见 `memory/papers/round-04-t1-long-record-event-confidence.md`、`memory/experiments/004-t1-long-record-event-confidence.md`。当前尚未实现或运行阈值评分，不得根据结果扩网格。
...
1. 第 4 轮按已冻结预注册实现和运行长记录事件级联合置信度最小反证；先复刻缓存 20 秒基线，再做源包选型、leave-one-file-out 稳定性、双向跨包和共同候选判定，不得扩展阈值或改配对规则。
2. T2 暂不继续 source-only residual、quantile/Huber、双模型平均或相同幅值拼接；只有无标签目标批次、解释偏移的元数据、DiTing/目标区标签或新的独立包出现时重开。
3. T3 暂不继续 NCM/top-m/中心化网格；只有实质不同的域不变机制、可靠无标签域移信号或新的独立包才重开。
4. 校正 `MASTER_PLAN.md`、`ROADMAP.md`、`训练与提分计划.md` 等旧文档中的部署待办、加速度积分、08 五分类和 API 未知等漂移，避免旧指令误导下一次发布。
