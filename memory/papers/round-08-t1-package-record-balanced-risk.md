...
- 日期：2026-08-12
- 状态：AnySearch MCP 发现、Playwright 原始页面核验、预注册和正式三折实验均已完成；候选被拒绝
...
正式实验绑定实现提交 `4edc4c5`。三折风险守恒全部通过，教师缓存与旧缓存 2,699 条记录逐条哈希相同，说明比较只改变训练风险。

候选的 12 单元 worst/mean 为 `-8.6889/-3.3602`，仅 2/12 正向，`robust_pass=false`。它把 R1 默认口径回退从基础 `KD+hard` 的 `-6.2722` 缩小到 `-2.3222`，但 R2/08 默认口径分别从 `+7.9222/+17.2722` 反转为 `-5.1944/-8.6889`。7 条长记录从基础 `KD+hard` 的 7/7 上升变为 1/7 上升、合计 `-14.7222`。

论文证据支持“保留域身份并审计最坏域”，但没有证明完全包等权、完全记录等权就是本任务正确的目标风险。实验反而表明两种极端都不安全：按窗口 ERM 使多窗记录贡献过大；完全记录等权又过度削弱长记录及其复杂事件结构。不得把本轮失败转化为权重截断、权重下限或长度分段网格；完整结果见 `memory/experiments/013-t1-package-record-balanced-distillation.md`。
