...
预注册候选在两个源包内部都选择相同的 `pca0_k40_s50`，重复嵌套 OOF 分别改善 `0.018247/0.008458 MAE`；但跨包 R1→R2 从 `0.621046` 恶化到 `0.621532`，R2→R1 从 `0.660726` 恶化到 `0.669618`，且两个方向 signed bias 都变大。距离闸覆盖约 `90–93%`，没有形成有效 OOD 隔离。

这与论文边界一致：source-only 包内 residual 可以学习回归收缩，却无法在 `P(y|x)` 或标签先验整体变化时推断目标截距方向。开发阶段失败后没有读取 08。完整结果见 `memory/experiments/003-t2-cross-package-residual-calibration.md`。
