...
- 发布链最终审计当时 `captured/manifest.jsonl` 行数和波形文件数均为 `0`。
- 随后只读复核发现 1 条 `/pick` 记录，包含 1 个 217 字节项，来源不是 loopback 且原始文件名不含 probe/smoke/test 标记；文件与 manifest 仍相互对应，因此按数据治理规则保留，不能按探针清理规则删除。
- 服务器生产虚拟环境中的 ObsPy 无法将该项解析为 MiniSEED，记录中的 API 响应为空对象；它不是可用训练波形，而是应保留的畸形公网输入审计样本。此次检查没有读取或输出原始内容。
- 同轮只读运维检查确认：公网 `/health` 为 HTTP 200/`ok`，没有 failed systemd unit，仓库所在文件系统可用空间约 230 GiB。
