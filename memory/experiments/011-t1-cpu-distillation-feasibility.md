...
后续实验 012 已完成真实平均概率缓存和三折 OOP 训练/评分。这里的 `132.1285057s` 必须继续解释为基于代表 annotation 窗成本的预算投影，不是全量物化实测。正式缓存实际用时：

```text
files                   = 2699
actual_elapsed_seconds  = 466.721230233
actual_probability_bytes= 65136648
manifest_sha256         = 645f8d04bc1a167f016594b94bbf5ed11495d8ebb51c0adac1bc7cb858555f5b
```

实际约为预算的 `3.53x`，因为完整运行还包含 ZIP/ObsPy 读取、逐模型调用、每文件写入、哈希和 `fsync`；但仍只有约 7.78 分钟，远低于 12 小时门槛，且实际概率字节与预算只差 738 字节。因此本轮的“可行”结论保持不变，但不得再把预算写成实际全量缓存耗时。

实验 012 最终证明训练工程可行但基础学生未通过 12 单元生产准入，详见 `memory/experiments/012-t1-phasenet-lopo-distillation.md`。服务器只作为实验计算节点；最终部署、systemd、cron、服务重启、捕获权限与生产发布均不属于这两轮。
