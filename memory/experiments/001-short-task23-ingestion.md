...
- `status`: accepted_deployed_verified
- `commit`: `53117b9d5912578c93d90e0a53774d66d0286aef`

## 服务器发布验证

- 2026-08-11 将服务器仓库从 `47fb72c655896913c656c0765acde57bd334c3a4` 快进到本轮提交 `53117b9d5912578c93d90e0a53774d66d0286aef`，服务器工作树核验为空。
- 重启系统级 `phasepick-api.service` 后，服务为 `active` 且 `enabled`；启动日志确认七成员 T1、SeismicXM T2/T3 与生产后处理参数均已加载，预热完成。
- 服务器回环与独立外网 `/health` 均返回 `{"status":"ok"}`。
- 使用 2 秒、100 Hz、三分量合成 MSEED 进行线上任务隔离烟测，样本 SHA-256 为 `6c05420f02bba3a183a2f7b171703df5c588d041d63c31c37b28988d1a23150e`：
  - `/classify`：HTTP 200，`{"SMK2": 5}`，客户端实测约 145.9 ms；
  - `/magnitude`：HTTP 200，`{"SMK2": 4.81}`，客户端实测约 113.5 ms；
  - `/pick`：HTTP 200，`{}`，客户端实测约 3.2 ms。
- 三条烟测捕获已按原始文件名精确识别并移入独立测试归档；生产 `captured/` 目录复核为空，没有移动或删除正式流量。
- 临时增量 Git bundle 已从服务器删除；本地忽略目录中的可恢复副本仍保留。当前发布结论为“代码、服务和短窗边界均已线上验证”。
