...
- 部署侧已无模型阻塞项。watchdog 的“随机令牌 + 直接 loopback 对端”双重捕获隔离
  已在服务器完成双向验收：认证探针不采集，公网伪造 token/代理头仍采集；唯一 root
  5 分钟 cron 的手动与真实定时触发均为 OK。旧版本回滚、四端点验证和再前滚也已完成。
  当前剩余外部操作是由用户在比赛窗口核对/登记各任务 URL，本次没有修改平台配置。
